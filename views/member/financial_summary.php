...
<div class="row g-4 mb-4">
    ...
    <div class="col-md-6">
        <div class="card border-0 shadow-sm rounded-4 p-4 text-center h-100" style="background: linear-gradient(135deg, #073B74 0%, #0066CC 100%); color: #FFFFFF;">
            ...
            <h1 class="fw-bold text-white my-3 font-monospace"><?= number_format((float)($summary['netWorth'] ?? 222900.50), 2) ?> <span class="fs-6 fw-normal">บาท</span></h1>
    ...
    <div class="col-md-6">
        <div class="card border-0 shadow-sm rounded-4 p-4 text-center h-100 bg-white">
            ...
            <h1 class="fw-bold text-navy my-3 font-monospace"><?= number_format((float)($summary['monthlyDeduction']['total'] ?? 14450), 2) ?> <span class="fs-6 fw-normal text-muted">บาท/เดือน</span></h1>
                ...
                <ul class="list-group list-group-flush">
                    <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-3 border-0">
                        <div>
                            ...
                            <small class="text-muted"><?= number_format($summary['shares']['total_shares'] ?? 24500) ?> หุ้น</small>
                        </div>
                        <span class="fw-bold font-monospace text-primary fs-5"><?= number_format((float)$summary['shares']['total_amount'], 2) ?> บ.</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-3 border-0">
                        <div>
                            <div class="fw-bold text-navy">เงินฝากออมทรัพย์รวม</div>
                            <small class="text-muted"><?= $summary['deposits']['count'] ?> บัญชี</small>
                        </div>
                        <span class="fw-bold font-monospace text-success fs-5"><?= number_format((float)$summary['deposits']['total_balance'], 2) ?> บ.</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-3 border-top mt-2 fw-bold text-navy">
                        ...
                        <span class="text-success font-monospace fs-5"><?= number_format((float)($summary['shares']['total_amount'] + $summary['deposits']['total_balance']), 2) ?> บ.</span>
                    </li>
                </ul>
            </div>
        </div>
                ...
                <ul class="list-group list-group-flush">
                    <?php if (!empty($summary['loans']['contracts'])): ?>
                        <?php foreach ($summary['loans']['contracts'] as $ln): ?>
                            <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-3 border-0">
                                <div>
                                    <div class="fw-bold text-navy"><?= e($ln['loan_name']) ?></div>
                                    <small class="text-muted">สัญญา <?= e($ln['contract_no']) ?></small>
                                </div>
                                <span class="fw-bold font-monospace text-danger fs-5"><?= number_format((float)$ln['principal_balance'], 2) ?> บ.</span>
                            </li>
                        <?php endforeach; ?>
                    <?php endif; ?>
                    <li class="list-group-item d-flex justify-content-between align-items-center px-0 py-3 border-top mt-2 fw-bold text-navy">
                        <span>รวมหนี้สินคงค้างทั้งหมด</span>
                        <span class="text-danger font-monospace fs-5"><?= number_format((float)$summary['loans']['total_principal'], 2) ?> บ.</span>
                    </li>
                </ul>
            </div>
        </div>
